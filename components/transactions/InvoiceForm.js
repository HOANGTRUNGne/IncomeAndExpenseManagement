...
const InvoiceForm = (props) => {
        const { title, onCreate, onUpdate, editing, categoriesData = [], handleModalForm} = props
        const {selectRecord = {}, isOpen} = editing
        const {category, type, name, amount, transaction_date, objectId} = selectRecord
        ...
        return (
            <Modal title={objectId ? `Update ${title}` : `Add ${title}`} open={isOpen} onCancel={() => handleModalForm()}
